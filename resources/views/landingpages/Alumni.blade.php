            ...
            <div class="row gy-4">
                ...
                <div>
                    <div class="visi-misi-content">
                        <h1>Data Alumni Sekolah Dasar Negeri Peraduan Waras</h1>
                        <form action="{{ route('Alumni') }}" method="GET" class="search-form">
    <input type="text" name="search" value="{{ request()->search }}" placeholder="Cari berdasarkan Nama Dan NISN" class="search-input"/>
    <button type="submit" class="search-button">Cari</button>
</form>
@if ($siswa->isEmpty())
            <p class="text-left">
                Tidak ada Nama dan NISN ditemukan dengan "<strong>{{ request('search') ?? '' }}</strong>"
                @endif
    ...
    @if($siswa->isNotEmpty())
        @foreach ($siswa as $index => $item)
            <tr>
                ...
                <td>
                    <span class="badge @if ($item->status == 'lulus') bg-success @elseif ($item->status == 'belum lulus') bg-warning @else bg-danger @endif">
                        {{ ucfirst($item->status) }}
                    </span>
                </td>
            </tr>
    ...
    @else
        ...
        <tr>
            <td colspan="10" class="text-center">Tidak ada data siswa ditemukan</td>
        </tr>
    @endif
</tbody>
