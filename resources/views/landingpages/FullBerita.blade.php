...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="main-content">
            <div class="row gy-4">
            <form action="{{ route('berita_all.index') }}" method="GET" class="search-form">
    <input type="text" name="search" value="{{ request()->search }}" placeholder="Cari berdasarkan judul" class="search-input"/>
    <button type="submit" class="search-button">Cari</button>
</form>
@if ($berita->isEmpty())
            <p class="text-left">
                Tidak ada judul ditemukan dengan "<strong>{{ request('search') ?? '' }}</strong>"
                @endif
                <!-- News Card 1 -->
                ...
                <div class="col-lg-3 col-md-6 col-sm-12">
                    <div class="card news-card h-100 shadow-sm">
                        <!-- Image Placeholder -->
                        <a href="{{ route('image.show', $item->file->id) }}">
                        <img
                        ...
                        <div class="card-body d-flex flex-column">
                            <h5 class="news-title text-truncate">{{ $item->judul }}</h5>
                            <p class="news-content text-truncate">
                            {{ Str::limit($item->content, 100) }}
                            </p>
                            <div class="mt-auto">
                                <a href="{{ route("berita_id.show", $item->id) }}" class="btn btn-primary btn-sm btn-read-more w-100">
                                    Baca Selengkapnya
                                </a>
                            </div>
                        </div>
